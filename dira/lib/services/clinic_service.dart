...
class ClinicService {
  ...
  static Future<String> createClinic(String name) async {
    final user = FirebaseAuth.instance.currentUser!;
    final clinicRef = _db.collection('clinics').doc();
    ...
    batch.set(clinicRef.collection('members').doc(user.uid), {
      'name': user.displayName ?? user.email,
      'email': user.email,
  ...
  static Future<void> ensureOwnerMembership(String clinicId) async {
    ...
    if (clinicDoc.data()?['ownerId'] != user.uid) return;
  ...
  static Future<void> updateMyPlayerId(String clinicId, String playerId) async {
    final uid = FirebaseAuth.instance.currentUser?.uid;
    if (uid == null) return;
    await _db.collection('clinics').doc(clinicId).collection('members').doc(uid).set({
      'oneSignalPlayerId': playerId,
    }, SetOptions(merge: true));
  }

  static Future<void> _deleteAllDocs(CollectionReference ref) async {
